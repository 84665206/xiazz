...
<head>
	<title>订单收货地址</title>
	<meta name="keywords" content="订单收货地址"/> 
	<meta name="description" content="订单收货地址"/>
    	...
    	<#if customerDeliveries??&&(customerDeliveries?size>0)>
		    ...
		    <li>
			    <div class="item-addr bdb-1px ">
			        <div class="ia-m" onclick="setDefaultDelivery(${deliverie?if_exists.id});">
			          <div class="mt_new">
			        ...
			        <div class="ia-m">
			        <label class="addck"><input type="radio" name="RadioGroup1" value="单选" <#if (deliverie.is_default)??&&(deliverie.is_default==0)>onclick="setDefaultDelivery(${deliverie?if_exists.id});"</#if>><span <#if (deliverie.is_default)??&&(deliverie.is_default==1)>class="on" <#else>class="moren" onclick="setDefaultDelivery(${deliverie?if_exists.id});"</#if>></span> 设为默认</label>
