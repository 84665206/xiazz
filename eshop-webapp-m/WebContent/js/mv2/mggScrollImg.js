    ...
    */
    ...
    auto_scroll : function(){//自动滚动
        ...
        self.timer = setTimeout(function(){
            self.go_index(self.index+1,null);
        },self.auto_wait_time);
    },
    go_index : function(ind,front){//滚动到指定索引页面
    	//判断当前图片懒加载情况
    	var lazyIndex = 0;
    	if(front=="pre"){
    		//如果是向前一页
    		if(ind == -1){
        		$(".switch-tab").children().eq(0).attr('class', null);
        		$(".switch-tab").children().eq(this.size-1).attr('class','current'); 
        		lazyIndex = this.size-1;
        	}else if(ind == (this.size-2)){
        		$(".switch-tab").children().eq(this.size-1).attr('class', null);
        		$(".switch-tab").children().eq(this.size-2).attr('class','current'); 
        		lazyIndex = this.size-2;
        	}else{
        		$(".switch-tab").children().eq(ind+1).attr('class', null);
        		$(".switch-tab").children().eq(ind).attr('class','current'); 
        		lazyIndex = ind;
        	}
    		...
    		if(ind == this.size){
    			$(".switch-tab").children().eq(this.size-1).attr('class', null);
        		$(".switch-tab").children().eq(0).attr('class','current'); 
        		lazyIndex = 0;
    		}else{
    			$(".switch-tab").children().eq(ind-1).attr('class', null);
        		$(".switch-tab").children().eq(ind).attr('class','current');
    ...
    imgLazy:function(lazyIndex){
    	//判断当前下标 图片是否已经懒加载
    	//若已经加载过咋不在执行
    	var lazyImg = $(".imgbox ul li a img");
    	$.each(lazyImg,function(){
    		if($(this).attr("index")==lazyIndex && 
    				$(this).hasClass("imgLazying")
    				&& $(this).attr("l")!="l"){
    			$(this).attr("l","l");
    			$(this).removeClass("imgLazying");
    ...
    next : function(){//下一页滚动
        if(!this.busy){
            this.go_index(this.index+1,null);
        }
    },
    prev : function(){//上一页滚动
        if(!this.busy){
            this.go_index(this.index-1,"pre");
        }
    },
